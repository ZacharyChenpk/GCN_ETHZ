...
def bi_inlinks(ent_inlinks_dict):
    news = {}
    for who, neighbors in ent_inlinks_dict.items():
        for n in neighbors:
            if (n not in ent_inlinks_dict) or (who not in ent_inlinks_dict[n]):
                if n not in news:
                    news[n]=[]
                news[n].append(who)
    for who, neighbors in news.items():
        if who not in ent_inlinks_dict:
            ent_inlinks_dict[who]=neighbors
        else:
            ent_inlinks_dict[who]+=neighbors
    return ent_inlinks_dict
